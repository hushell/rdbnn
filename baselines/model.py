...
class FeatureExtractor(nn.Module):
    def __init__(self, features=64, pretrained_resnet=False):
        super(FeatureExtractor, self).__init__()
        encoder = resnet18(pretrained=pretrained_resnet, input_channel=3, features=features)
        self.encoder = nn.Sequential(*list(encoder.children())[:-2])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))

    def forward(self, im):
        feature = self.encoder(im)
...
class Classifier(nn.Module):
    def __init__(self, inputs=512, num_classes=10):
